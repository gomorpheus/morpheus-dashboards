...
class ClusterCapacityWidget extends React.Component {
	...
	//data methods
	refreshData() {
		if (this.state.autoRefresh == true) this.loadData();
	}

	loadData(filter, options) {
		//load count
		var apiQuery =
			"stats(sum(stats.usedMemory),sum(maxMemory),avg(stats.usedCpu),sum(stats.usedStorage),sum(maxStorage)) usedCpu > 1";
	...
	setData(results) {
		...
		for (var i = 0; i < results.items.length; i++) {
			results.items[i].name = results.items[i].name.replace("stats.", "");
		}
	...
	render() {
		...
		if (this.state.data && this.state.data.loaded == true && this.state.data.items.length > 0) {
			...
			if (usedMemory) usedMemory = usedMemory[1];

			maxMemory = this.findDataItem("maxMemory");
			if (maxMemory) maxMemory = maxMemory[1];
			if (maxMemory > 0) {
				memoryPercentUsed = (usedMemory / maxMemory) * 100;
				memoryChartTitle =
					Morpheus.utils.formatBytes(usedMemory, 0) +
					" of " +
					Morpheus.utils.formatBytes(maxMemory);
				//set threshold
				if (memoryPercentUsed > 90) memoryChartColor = "red";
				else if (memoryPercentUsed > 75) memoryChartColor = "yellow";
			}

			//cpu
			cpuPercentUsed = this.findDataItem("usedCpu");
			if (cpuPercentUsed) cpuPercentUsed = cpuPercentUsed[1];
			if (cpuPercentUsed > 0) {
				cpuChartTitle = Math.round(cpuPercentUsed) + "%";
				//set threshold
				if (cpuPercentUsed > 90) cpuChartColor = "red";
				else if (cpuPercentUsed > 75) cpuChartColor = "yellow";
			}
			...
			if (fetchedUsed) usedStorage = fetchedUsed[1];
			if (fetchedMax) maxStorage = fetchedMax[1];
			if (maxStorage > 0) {
				storagePercentUsed = (usedStorage / maxStorage) * 100;
				storageChartTitle =
					Morpheus.utils.formatBytes(usedStorage, 0) +
					" of " +
					Morpheus.utils.formatBytes(maxStorage);
				//set threshold
				if (storagePercentUsed > 90) storageChartColor = "red";
				else if (memoryPercentUsed > 75) storageChartColor = "yellow";
			}
		}
		//render it
		return (
			<Widget>
				<WidgetHeader
					...
					icon="/assets/infrastructure/clusters.svg#Layer_1"
					link="/infrastructure/clusters"
				/>
				<div className="dashboard-widget-content">
					<div id={this.state.chartId} className="guage-chart-widget">
						<div
							className={"cpu-guage guage-md progress-bar-" + cpuChartColor}
							title={cpuChartTitle}
						>
						...
						<div
							className={"memory-guage guage-md progress-bar-" + memoryChartColor}
							title={memoryChartTitle}
						>
							<svg viewBox="0 0 100 100" width="100" height="100">
								<circle cx="50" cy="50" r="40" className="chart-bg" />
								<circle
						...
						<div
							className={"storage-guage guage-md progress-bar-" + storageChartColor}
